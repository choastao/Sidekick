    ...
    ILogger<PobItemTextService> logger)
{
    ...
    /// **不要**用空表兜底 —— 空表只打死了词缀模板：每条 stat 都会变成「我们没认出」
    /// （缺口计数**报满**、不是 0），而送出去的文本里词缀一条不剩（只剩稀有度 / 名字 / 基底 / 属性行，
    /// 见 <see cref="PobItemText.Build"/> 的前半段）——试穿出来的数字是拿这样的文本算的，
    /// 表面还是一次「成功」，结论却建立在错误的前提上。
    /// </summary>
