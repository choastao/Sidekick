...
public static class ResistanceGuardrail
{
    ...
    ///      ⚠ 已知局限（因果方向别写反了）：
    ///        · 上限被**抬到 75 以上**、当前正好卡在那个上限（溢出 = 0）那一档**不会漏**——
    ///          它被第 ② 条兜住了（有效抗性 = 该上限 ≥ 75 &gt; 74.5）。第 ② 条在这种情形下会**多报**
    ///          一次 Tradeoff（有效抗性落在 [74.5, 上限) 时也判成「到上限」）；
    ///        · 真会漏的是**上限被压到 74.5 以下**那一档（有效抗性 = 该上限 &lt; 74.5、溢出 = 0）。
    ///      取舍：宁可漏判 / 多报，也不把「不知道」当「到上限」——那会把正常取舍说成缺口变糟。
    ///
